...
//===============================================================================================
    ...
    public function stuAttend()
    {
        ...
        $attend = new NightDetailModel();
        $cond = array();
        $time = date("Y-m-d 00:00:00",time());
        $cond['time'] = array("gt",$time);
        $attend_list = $attend->getNightDetail($cond);
        ...
        foreach($list as $k=>$v)
        {
            ...
            else if((int)$v['name'] == 2)
            {
                $lists['l2'][] = $list[$k];
            }
            else if((int)$v['name'] == 3)
            {
                $lists['l3'][] = $list[$k];
            }
            else if((int)$v['name'] == 4)
            {
                $lists['l4'][] = $list[$k];
            }
            else
        ...
        // echo "<pre>";
        // var_dump($lists);
        // var_dump($attend_list);
        // die;
        $this->assign('class',$lists);
    	$this->display('stuAttend');
    }

    public function getNightRecord()
    {
        $type = @$_POST['typ'];
        $return = array();
        if($type == 'json')
        {
            ...
            $record = new NightRecordModel();
            $record_list = $record->getNightRecord($cond);
            // var_dump($cond);
    ...
    public function stuNightDetail()
    {
        $class = new ClassModel();
