...
foreach($url_arr as $key=>$url)
{
	...
	if(!empty($return11->camera_list))
	{
		...
		foreach($camera_list as $k=>$v)
		{
			...
			echo "班级的相机";
			$class_sql = "SELECT * FROM syllabus where room='".$k."' AND time='7' AND week='".date('w')."'";
			...
			if(!empty($class_stu))
			{
				...
				if(!empty($face_res->alarm_list))
				{
					foreach($face_res->alarm_list as $kkk=>$vvv)
					{
						if(!in_array($vvv->search_list[0]->person_id,$true_ids) && in_array($vvv->search_list[0]->person_id,$attend_ids))
						{
							...
							$initsql = "INSERT INTO attend_detail(id,stu_id,room,time,type,week,weektime) VALUES(NULL,'".$vvv->search_list[0]->person_id."','".$k."','".$vvv->alarm_time."',1,'".date('w')."',7)";
							mysql_query($initsql);
						}
					}
				}
				...
				if(!empty($dis_ids))
				{
					...
					{
						$initsql = "INSERT INTO attend_detail(id,stu_id,room,time,type,week,weektime) VALUES(NULL,'".$val."','".$k."','".date("Y-m-d H:i:s",time())."',3,'".date('w')."',7)";
						mysql_query($initsql);
					}
				}
				$detail_sql = "INSERT INTO attend_record(id,room,date,week,time,total,attend,absent,absent_id,lesson) VALUES(null,'".$k."','".date('Y-m-d')."','".date('w')."','7','".count($attend_ids)."','".count($true_ids)."','".count($dis_ids)."','".@implode(',',$dis_ids)."','".$lesson_id."')";
...
$time .= "星期".date('w').".........";
file_put_contents("lesson7.txt",$time,FILE_APPEND);
mysql_close($con);
