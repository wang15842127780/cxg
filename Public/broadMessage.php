...
class BroadMessage{
	public function broadMessage($user_id,$desc)
	{
		$secret = '+qmsXYuCfGBWdPvmaHuEJg==';
		$package = 'com.jun.dn.cxg';
		...
		// $targetMessage->setTarget('topic', TargetedMessage::TARGET_TYPE_USER_ACCOUNT); // 设置发送目标。可通过regID,alias和topic三种方式发送
		...
		// print_r($sender->broadcastAll($message1, $aliasList)->getRaw());
		$res = $sender->sendToUserAccount($message2, $user_id)->getRaw();
		return $res;
	}
}
